...
async def _retry_with_pow(base_url: str, headers: dict, payload: dict) -> AsyncGenerator[dict, None]:
    try:
                ...
                async for line in resp.aiter_lines():
                    ...
                    if line.startswith("data:"):
                        ...
                        if data.get("type") == "pow":
                            ...
                            async for event in _retry_with_pow(DEEPSEEK_BASE_URL, headers, payload):
                                yield event
                            return
